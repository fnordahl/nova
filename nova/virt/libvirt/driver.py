    ...
    def snapshot(self, context, instance, image_id, update_task_state):
        ...
        try:
            ...
            self._image_api.update(context, image_id, metadata,
                                   purge_props=False)
        except (exception.ImageUnacceptable, exception.Forbidden) as e:
            ...
            snapshot_backend.cleanup_direct_snapshot(failed_snap, destroy=True,
                                                     ignore_errors=True)
            update_task_state(task_state=task_states.IMAGE_PENDING_UPLOAD,
                              expected_state=task_states.IMAGE_UPLOADING)

            # Standard snapshot uses qemu img-convert which is not safe to run
            # with live_snapshot.
            live_snapshot = False
            ...
            with utils.tempdir(dir=snapshot_directory) as tmpdir:
                try:
                    ...
                    if live_snapshot:
                        ...
                        self._live_snapshot(context, instance, virt_dom,
                                            disk_path, out_path,
                                            image_format, base)
                    else:
